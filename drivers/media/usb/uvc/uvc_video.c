...
#if defined(CONFIG_ARCH_BSP) && defined(UVC_HOST_SLICE_MODE)
...
#if defined(CONFIG_ARCH_BSP) && defined(UVC_HOST_SLICE_MODE)
#define UVC_STREAM_RES_FLAG	0x80000000
#define UVC_STREAM_EOF_FLAG	0x40000000
static void uvc_video_decode_end(struct uvc_streaming *stream,
		struct uvc_buffer *buf, const u8 *data, int len)
{
	u8 is_eof = (data[1] & UVC_STREAM_EOF) ? 1 : 0;
	u8 is_res = (data[1] & UVC_STREAM_RES) ? 1 : 0;

	/* Mark the buffer as done if the EOF marker is set. */
	if ((is_eof || is_res) && buf->bytesused != 0) {
		uvc_trace(UVC_TRACE_FRAME, "Frame complete (EOF found).\n");
		if (data[0] == len)
			printk(KERN_INFO"EOF in empty payload.\n");

		if (is_res)
			buf->buf.flags |= UVC_STREAM_RES_FLAG;
		else
			buf->buf.flags &= (~UVC_STREAM_RES_FLAG);

		if (is_eof)
			buf->buf.flags |= UVC_STREAM_EOF_FLAG;
		else
			buf->buf.flags &= (~UVC_STREAM_EOF_FLAG);

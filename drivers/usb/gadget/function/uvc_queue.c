...
#if defined(CONFIG_ARCH_BSP) && defined(CONFIG_USB_F_UVC_STILL_IMAGE)
	struct uvc_video *video = container_of(queue, struct uvc_video, queue);
	/* Still Image method2 reconfig endpoint capability may cause video transfer fail,
	 * it doesn't mean error or disconnect in Still Image status, we do not cancel
	 * the irqqueue so that the video transfer can be resumed after Still Image status.
	 */
	if (video->max_payload_size == 0 && uvc_still_is_md2(video->still) && uvc_still_is_active_status(video->still))
		return;
#endif

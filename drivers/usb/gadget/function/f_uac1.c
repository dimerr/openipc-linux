...
static struct usb_descriptor_header *f_audio_playback_desc[] = {
	(struct usb_descriptor_header *)&uac_iad,
	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc,
...
static struct usb_descriptor_header *f_audio_ss_playback_desc[] = {
	(struct usb_descriptor_header *)&uac_iad,
	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc,
...
static struct usb_descriptor_header *f_audio_capture_desc[] = {
	(struct usb_descriptor_header *)&uac_iad,
	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc,
...
static struct usb_descriptor_header *f_audio_ss_capture_desc[] = {
	(struct usb_descriptor_header *)&uac_iad,
	(struct usb_descriptor_header *)&ac_interface_desc,
	(struct usb_descriptor_header *)&ac_header_desc,
...
#ifdef CONFIG_ARCH_BSP
#define UAC_SINGLE_INTERFACE_NUMBER 1
static int select_single_interface(int mode, int status, struct f_uac1 *uac1,
		struct usb_descriptor_header ***desc, struct usb_descriptor_header ***desc_ss)
{
	ac_header_desc.wTotalLength -= cpu_to_le16(UAC_DT_INPUT_TERMINAL_SIZE +
			UAC_DT_OUTPUT_TERMINAL_SIZE + UAC_SINGLE_INTERFACE_NUMBER);
	ac_header_desc.bInCollection = UAC_SINGLE_INTERFACE_NUMBER;
	ac_header_desc.bLength = UAC_DT_AC_HEADER_SIZE(UAC_SINGLE_INTERFACE_NUMBER);
	uac_iad.bInterfaceCount -= UAC_SINGLE_INTERFACE_NUMBER;

	ac_header_desc.baInterfaceNr[0] = status;
	if (mode == UAC_PLAYBACK_ONLY) {
		as_in_interface_alt_0_desc.bInterfaceNumber = status;
		as_in_interface_alt_1_desc.bInterfaceNumber = status;
		uac1->as_in_intf = status;
		uac1->as_in_alt = 0;
		*desc = f_audio_playback_desc;
		*desc_ss = f_audio_ss_playback_desc;
	} else if (mode == UAC_CAPTURE_ONLY) {
		as_out_interface_alt_0_desc.bInterfaceNumber = status;
		as_out_interface_alt_1_desc.bInterfaceNumber = status;
		uac1->as_out_intf = status;
		uac1->as_out_alt = 0;
		*desc = f_audio_capture_desc;
		*desc_ss = f_audio_ss_capture_desc;
	}

	return 0;
}

static int config_uac_mode(int mode, struct usb_configuration *c, struct usb_function *f,
		struct usb_descriptor_header ***desc, struct usb_descriptor_header ***desc_ss)
{
	int status = 0;
	struct f_uac1 *uac1 = func_to_uac1(f);
	...
	if (mode == UAC_PLAYBACK_ONLY || mode == UAC_CAPTURE_ONLY) {
		return select_single_interface(mode, status, uac1, desc, desc_ss);
	}
...
static int f_audio_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	int				status;
#ifdef CONFIG_ARCH_BSP
	struct usb_descriptor_header **cur_desc = (struct usb_descriptor_header **)f_audio_desc;
	struct usb_descriptor_header **cur_ss_desc = (struct usb_descriptor_header **)f_audio_ss_desc;
#endif
...
#ifdef CONFIG_ARCH_BSP
	status = config_uac_mode(audio_opts->uac_mode, c, f, &cur_desc, &cur_ss_desc);
	if (status < 0)
		goto fail;
#else
	status = usb_interface_id(c, f);
...
#ifdef CONFIG_ARCH_BSP
	status = usb_assign_descriptors(f, cur_desc, cur_desc,
				cur_ss_desc, NULL);
#else
...
#ifdef CONFIG_ARCH_BSP
	audio->params.uac_mode = audio_opts->uac_mode;
#endif
...
#ifdef CONFIG_ARCH_BSP
	opts->uac_mode = UAC_PLAYBACK_CAPTURE;
#endif

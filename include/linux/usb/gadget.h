...
#if defined(CONFIG_ARCH_BSP)
	/* Currently, most standard reqs (such as clear endpoint halt) just handles
	 * by controller driver, and other driver (gadget/function) can not catch.
	 * Under some kind of scene (bulk mode uvc), the function driver should capture
	 * the reqs to do something (stream off).
	 */
	unsigned			notify_standard_req_to_function:1;
#endif
	int				irq;
};
